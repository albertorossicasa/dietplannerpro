...
export default function EditClientScreen({ route, navigation }) {
  ...
  const [initialLoading, setInitialLoading] = useState(!initialClient);
  const [form, setForm] = useState(initialClient || {});

  useEffect(() => { if (!initialClient) loadClient(); }, []);

  const loadClient = async () => { const data = await Database.getCliente(clientId); setForm(data); setInitialLoading(false); };
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true);
    await Database.updateCliente(clientId, form);
    Alert.alert('Successo', 'Cliente aggiornato!', [{ text: 'OK', onPress: () => navigation.goBack() }]);
    setLoading(false);
  };
  ...
  return (
    <ScrollView style={styles.container}>
      <View style={styles.form}>
        ...
        <Text style={styles.label}>Data Nascita</Text><TextInput style={styles.input} value={form.data_nascita} onChangeText={v => handleChange('data_nascita', v)} />
        <Text style={styles.label}>Sesso</Text>
        <View style={styles.sexRow}>
          <TouchableOpacity style={[styles.sexButton, form.sesso === 'M' && styles.sexButtonActive]} onPress={() => handleChange('sesso', 'M')}><Text>Maschio</Text></TouchableOpacity>
          <TouchableOpacity style={[styles.sexButton, form.sesso === 'F' && styles.sexButtonActive]} onPress={() => handleChange('sesso', 'F')}><Text>Femmina</Text></TouchableOpacity>
        </View>
        <Text style={styles.label}>Città</Text><TextInput style={styles.input} value={form.citta} onChangeText={v => handleChange('citta', v)} />
        <Text style={styles.label}>Telefono</Text><TextInput style={styles.input} value={form.telefono} onChangeText={v => handleChange('telefono', v)} />
        <Text style={styles.label}>Email</Text><TextInput style={styles.input} value={form.email} onChangeText={v => handleChange('email', v)} />
        ...
        <Text style={styles.label}>Altezza (cm)</Text><TextInput style={styles.input} value={form.altezza_cm?.toString()} onChangeText={v => handleChange('altezza_cm', v)} keyboardType="numeric" />
        <Text style={styles.label}>Peso (kg)</Text><TextInput style={styles.input} value={form.peso_kg?.toString()} onChangeText={v => handleChange('peso_kg', v)} keyboardType="numeric" />
        ...
        <CustomButton title={loading ? 'Salvataggio...' : '💾 SALVA MODIFICHE'} onPress={handleSubmit} loading={loading} />
